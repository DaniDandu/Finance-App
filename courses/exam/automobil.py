...
class Automobil(Vehicul):
    ...
    def print_caracteristici(self):
        print(f"Caracteristici:\n"
              f"    Motorul are {str(self.__motor.get_nr_cilindri())} cilindri, "
              f"puterea maxima este {str(self.__motor.get_putere_max())} "
              f"si are catalizatorul {str(self.__motor.get_catalizator())}.\n"
              f"    Masina are {str(self.get_nr_roti())} roti, "
              f"capacitatea rezervorului de {str(self.get_capacitate_rezervor())} litri "
              f"si consumul urban de {str(self.get_consum_urban())}.\n"
